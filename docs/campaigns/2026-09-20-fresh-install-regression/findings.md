...
**Fifteen rows as of 2026-09-20, two of them P1 found by the large sample Task.** F-001 to F-005 were pre-filed while authoring
...
| F-012 | **P1** | X-3 chain · Delivery workflow | **The Delivery workflow opens pull requests containing failing tests, and every reviewer reports green.** No node in the graph ever executes the test suite. | ATL-5 delivered PR #19 (`atlas-sdlc-sandbox`, 58/59 pass) and PR #2 (`atlas-sdlc-sandbox-web`, 34/35 pass). Both repos are green on `main` (43/43 and 10/10), so both failures are regressions introduced by the delivery. The reviewers state their own scope: QA Reviewer — *"Validator green (**structural checks only**)"*; Automation Reviewer — *"Ran `node --check test/todo.atl-10.test.js` — syntax clean"*. `node --check` parses without executing. The chain verifies CSV row shape, verbatim test-name matching, `tested_by` links and anti-patterns, but never runs `npm test`. | 6 error, chain | open |
