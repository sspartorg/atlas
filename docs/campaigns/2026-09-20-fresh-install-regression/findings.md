...
| F-014 | P2 | B2 · `/tasks/new` | ⚠️ **UNREPRODUCED.** React threw `Maximum update depth exceeded` from the New Task form's input `onChange`, eight times across two separate sessions. Not reproduced on demand. | Stack: `onChange (src/pages/TaskNew.tsx:376-377)` → MUI `InputBase handleChange` → `dispatchSetState` → `enqueueConcurrentHookUpdate`. Captured 2026-09-20 at 14:59 (x4) and 17:06 (x4); both clusters coincide with typing into the Title/Description fields while creating ATL-1. The on-disk handlers are plain `setTitle(e.target.value)` / `setDescription(e.target.value)` (`TaskNew.tsx:273,314`) and the only effect is `useDraftGuard(Boolean(title.trim() || description.trim()))` at `:115`, whose dependency is a boolean that flips once - none of which should loop. **Attempts that did NOT reproduce it:** paced typing (1 char / 2s), a 33-char burst, and select-all + Delete + retype. Correlation worth noting but not asserted as cause: during the 17:06 cluster the typed text arrived corrupted ('printsthe', 'Kee it to', 'oher') - consistent with dropped updates, and initially misattributed by the campaign to synthetic keystroke injection. Needs a repro before it can be fixed; re-check during task-09's wave B walk. | 6 error | open - needs repro |
