...
| G-004 | **P1** | `api` · agent outcome routing | Reviewer verification gates remain unenforced in principle. The predecessor populated the empty `checklists.json` files, which closed the symptom, but the router still reads the agent's own claim rather than a script's exit code — so an agent that asserts a red suite is green is still believed. | `packages/api/src/services/agent-runner-outcome-routing.ts:72-88`: an empty required checklist returns `apply_on_pass`, and `if (item.passed)` trusts the `atlas-outcome` block. The machinery to do better exists and is unused — guardrail script `coder-tests-green` runs the declared `test` script under `--run-tests`, and `agent-code-reviewer/prompt.md:34` already instructs the agent to run it. Nothing consults its exit code. Predecessor F-012 records the consequence: ATL-5 shipped two PRs with red suites while every reviewer reported green. | open — task-04 |
