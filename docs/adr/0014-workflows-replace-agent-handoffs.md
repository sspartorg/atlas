...
- **Test children route by link, not by prompt.** MCP `create_item` has no `workflow_id`. Instead the End node has `test_child_workflow_id`: a child with an outgoing `tested_by` link (PO Writer's `[QA]` twin → dev story) goes there, every other child to `child_workflow_id`; a child that already has a `workflow_id` (set by the Owner via `PUT /api/items/:id/workflow`) keeps it. `planning` ships `test_child_workflow_id: template:qa`. Children are matched in `draft` or `ready`; nothing forces them to `draft` during the run.
- **Routed children keep the parent in review.** An item whose run routed children ends `in_review`, not `done` (a planned epic isn't done until its stories are).
- **Dispatch skips blocked items.** The item_ready / schedule queue skips `ready` items with an open `depends_on` target instead of retrying the oldest one every tick (which stalled the whole queue).
- **`items.pr_url` is set** alongside the `item_external_links` row when End opens a PR — Automation reads the dev story's `pr_url` to check it merged.
- **Questions are asked only through the outcome.** The constitution and catalog prompts no longer tell agents to post a question comment; the `reason` of `asked_question` is posted by the runner. The park comment is posted with no `agent_id` (the thread labels it "Workflow").
- **Resuming refreshes the branch onto main.** Resuming at an Owner or agent node commits leftovers and re-runs `ensureWorktree`, which rebases the run's commits onto the latest default branch. Found in the Planning demo: the Quality run waited for the dev PR to merge, but its worktree predated the merge, so Automation's tests were written against code its branch did not have (PR #9 red on its own, green once merged).
- **Code Reviewer's validator runs the tests.** `check-coder-tests-green.sh <id> --run-tests` also runs the declared `test` script; a prompt-only instruction was skipped twice in real runs.
- **Restart recovery.** The periodic orphan reaper reaps rows that started before the API process booted, so a step killed by a restart parks within a minute instead of 30; an errored step's park reason carries the runner / reaper error line.
- **Coder and Code Reviewer work without an Architect step.** With no spec on the branch they use the story's acceptance criteria as the contract.
...
- **The step's completion comment carries its outcome.** `buildOrchestratorRunCompletedBody` renders the parsed `atlas-outcome` kind, reason and summary (it used to post only a run link). This is how a reviewer's rejection reason reaches the retried step. A park caused by a step's own `asked_question` / `rejected` reason posts no second "waiting for you" comment; the Owner notification still fires.
- **Real-agent check (2026-09-14, sandbox project, Claude Sonnet 4.6).** Development template on SDB-10: 4 steps, one branch, PR #6, ~$1.36. Architect → Coder ⇄ Code Reviewer on SDB-11: Architect asked JSON-or-CSV and parked; the Owner's reply re-ran it; Code Reviewer rejected on a README checklist row; the retried Coder closed that gap from the comment thread; End opened PR #7 (`loop_count` 1, ~$1.53). The catalog Coder refuses to start without an Architect spec (`waiting_on_architect`), so workflows need an Architect step before it.
