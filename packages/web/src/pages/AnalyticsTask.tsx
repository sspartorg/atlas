...
export function AnalyticsTask() {
    ...
    const summary = useQuery({
        queryKey: ['analytics-task', safeTaskId],
        queryFn: () => api.analytics.task(safeTaskId),
        enabled: Boolean(safeTaskId),
        staleTime: 30_000,
    });
        ...
        <Box sx={{ px: { xs: 3, md: 8 }, py: { xs: 6, md: 12 } }}>
            <Hero
                breadcrumb={
                    <>
                        ...
                        <Box
                            component={RouterLink}
                            to={`/analytics/project/${data.task.project_id}`}
                            sx={{
                                fontFamily: MONO,
                                fontSize: 11,
                                color: 'rgba(255,255,255,.65)',
                        ...
                        >
                            {data.task.project_name || data.task.project_id || 'Project'}
                        </Box>
                        <Typography
                            sx={{ fontFamily: MONO, fontSize: 11, color: 'rgba(255,255,255,.4)' }}
                        >
            ...
            {byKindPie.length > 0 && (
                <Card sx={{ mb: 4 }}>
                    <ChartTitle
                        ...
                        title="Cost split across the task's sub-tasks"
            ...
            <Card>
                <ChartTitle
                    eyebrow="Child items"
                    title="Per-item cost"
                    sub={`Every descendant of this task. Sorted by cost.${typeFilter ? ` Filtered to ${ITEM_TYPE_LABEL[typeFilter]} only.` : ''}`}
                />
