...
import { AnalyticsTask } from './AnalyticsTask.js';
...
const minimalTask = {
    task: { id: 'ATL-1', title: 'Task One', project_id: 'p1', project_name: 'Atlas' },
    summary: {
        run_count: 0,
        total_cost_usd: 0,
        input_tokens: 0,
...
const populatedTask = {
    ...
    byKind: [
        { type: 'sub_task', total_cost_usd: 1, item_count: 3, run_count: 4 },
        { type: 'task', total_cost_usd: 1, item_count: 2, run_count: 1 },
    ],
    totals: {
        run_count: 5,
        total_cost_usd: 2,
...
describe('AnalyticsTask page', () => {
    ...
    it('renders the no-id fallback when taskId is missing', () => {
        renderWithProviders(<AnalyticsTask />);
        expect(screen.getByText(/no task id/i)).toBeInTheDocument();
    });
    ...
    it('renders fallback project_id in breadcrumb when project_name is empty', async () => {
        // data.task.project_name is empty → breadcrumb shows project_id
        const noNameTask = {
            ...populatedTask,
            task: { id: 'ATL-1', title: 'Task One', project_id: 'p-fallback', project_name: '' },
        };
        server.use(
    ...
    it('renders "Project" fallback in breadcrumb when both project_name and project_id are empty', async () => {
        const noIdNoNameTask = {
            ...populatedTask,
            task: { id: 'ATL-1', title: 'Task One', project_id: '', project_name: '' },
        };
        server.use(
    ...
    it('renders "child item" (singular) when descendant_count is 1', async () => {
        const singleDescTask = { ...populatedTask, descendant_count: 1 };
        server.use(
    ...
    it('renders singular "item" and "run" on a kind card with counts of 1', async () => {
        const singleCountTask = {
            ...populatedTask,
            byKind: [{ type: 'sub_task', total_cost_usd: 1, item_count: 1, run_count: 1 }],
        };
        server.use(
    ...
    it('Tooltip formatter returns formatted cost and resolved label for known type', async () => {
        ...
        // Known type: payload.type = 'sub_task'
    ...
    it('Legend formatter returns ITEM_TYPE_LABEL for known types and raw string for unknown', async () => {
        ...
        // Known kinds
        expect(fmt('sub_task')).toBe('Sub-task');
        expect(fmt('task')).toBe('Task');
