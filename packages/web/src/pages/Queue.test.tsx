...
function entry(overrides: Partial<IWorkflowQueueEntry> = {}): IWorkflowQueueEntry {
    return { workflow: makeWorkflow({ max_parallel_runs: 2 }), running: [], waiting: [], queued: [], ...overrides };
}

const subtasksGraph = {
    nodes: [
        { id: 'start', type: 'start' as const, position: { x: 0, y: 0 } },
        { id: 'subs', type: 'subtasks' as const, sub_workflow_id: 'wf-build', position: { x: 1, y: 0 } },
...
const busy: IWorkflowQueue = {
    workflows: [
        entry({
            running: [
                run({ id: 'wfr-1', item_id: 'ATL-7', item_title: 'Add login', current_node_id: 'coder' }),
                run({ id: 'wfr-3', item_id: 'ATL-8', item_title: 'Fix bugs', current_node_id: 'subs', graph_snapshot: subtasksGraph }),
            ],
            waiting: [
                run({ id: 'wfr-2', item_id: 'ATL-5', item_title: 'Pick a DB', status: 'waiting_for_owner', park_reason: 'Postgres or SQLite?' }),
            ],
            queued: [makeTask({ id: 'ATL-3', title: 'Oldest', status: 'ready', workflow_id: 'wf-1' }), makeTask({ id: 'ATL-4', title: 'Next', status: 'ready', workflow_id: 'wf-1' })],
        }),
    ],
    unassigned: [],
};
...
function setup(queue: IWorkflowQueue, ...extra: Parameters<typeof server.use>) {
    ...
    server.use(
        ...extra,
        handlers.listProjects([makeProject({ id: 'p1', name: 'Atlas' }), makeProject({ id: 'p2', name: 'Blog' })]),
...
describe('Queue page', () => {
    it('shows the counter strip and points me at workflows when there are none', async () => {
        ...
        expect(screen.getByText('0 running · 0 queued · 0 waiting on you · 0 need a workflow')).toBeInTheDocument();
    ...
    it('renders a card per workflow with its slots, running, waiting and queued Tasks', async () => {
        ...
        const c = within(card);
        expect(c.getByRole('link', { name: 'Development' })).toHaveAttribute('href', '/workflows/wf-1');
        expect(c.getByText('Atlas · On item ready')).toBeInTheDocument();
        expect(c.getByText('2 / 2')).toBeInTheDocument();
        expect(c.getByRole('progressbar', { name: 'Development running slots' })).toHaveAttribute('aria-valuenow', '100');

        // Running: the step's agent name from the graph snapshot, or Sub-tasks.
        expect(c.getByRole('link', { name: /ATL-7\s*Add login/ })).toHaveAttribute('href', '/tasks/ATL-7');
        expect(c.getByText('Coder')).toBeInTheDocument();
        expect(c.getByText('Sub-tasks')).toBeInTheDocument();
        const runLinks = c.getAllByRole('link', { name: 'Open run' }).map((a) => a.getAttribute('href'));
        expect(runLinks).toEqual(['/workflows/wf-1/runs/wfr-1', '/workflows/wf-1/runs/wfr-3', '/workflows/wf-1/runs/wfr-2']);
        ...
        expect(screen.getByText('2 running · 2 queued · 1 waiting on you · 0 need a workflow')).toBeInTheDocument();
    });

    it('starts a queued Task when a slot is free', async () => {
        let body: unknown;
        const { user } = setup(
            { workflows: [entry({ queued: [makeTask({ id: 'ATL-3', status: 'ready', workflow_id: 'wf-1' })] })], unassigned: [] },
    ...
    it('explains why queued Tasks wait on a paused workflow, and resumes it from the switch', async () => {
        ...
        const { user } = setup(
            { workflows: [entry({ workflow: paused, queued: [makeTask({ id: 'ATL-3', status: 'ready', workflow_id: 'wf-1' })] })], unassigned: [] },
            http.patch(`${BASE}/workflows/wf-1`, async ({ request }) => {
                body = await request.json();
                return HttpResponse.json(makeWorkflow());
            }),
        );
        expect(await screen.findByText('Paused: these wait until you turn it back on.')).toBeInTheDocument();
    ...
    it('shows an empty card for an idle workflow', async () => {
        ...
        expect(await screen.findByText('Nothing running or queued. Set a ready Task’s workflow to this one to queue it.')).toBeInTheDocument();
    });
    ...
    it('queues a ready Task that no workflow will pick up', async () => {
        ...
        const { user } = setup(
            {
                workflows: [entry()],
                unassigned: [makeTask({ id: 'ATL-9', title: 'Loose', status: 'ready' }), makeTask({ id: 'BLG-1', project_id: 'p2', title: 'Other', status: 'ready' })],
            },
        ...
        expect(within(section).getByRole('link', { name: 'Create a workflow' })).toHaveAttribute('href', '/workflows');
    ...
    it('shows an error when the queue fails to load', async () => {
        setup({ workflows: [], unassigned: [] }, http.get(`${BASE}/workflow-queue`, () => HttpResponse.json({ error: 'boom' }, { status: 500 })));
