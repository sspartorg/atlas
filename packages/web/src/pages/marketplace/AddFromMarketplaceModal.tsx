...
import { useMarketplaceAgentFull } from '../../hooks/useMarketplace.js';
...
function InstallHints({ agentId }: { agentId: string }) {
    const { data: full } = useMarketplaceAgentFull(agentId);
    return <CliUnavailableAlert cli={full?.agent.cli} beforeInstall sx={{ mb: 3 }} />;
}
