...
describe('App', () => {
    ...
    it('redirects to /onboarding when onboarding_complete is 0', async () => {
        ...
        await waitFor(() => expect(document.location.pathname).toBe('/'), { timeout: 5000 });
    });
    ...
    it('AppShell: registers service worker when supported', async () => {
        ...
        try {
            ...
            await waitFor(() => expect(registerSpy).toHaveBeenCalledWith('/sw.js'), {
                timeout: 3000,
            });
        } finally {
    ...
    it('AppShell (desktop): onShortcutsOpen — Topbar Shortcuts button opens ShortcutsDialog (L138)', async () => {
        ...
        const shortcutsBtn = Array.from(shortcutBtns).find((el) =>
            el.textContent?.includes('Shortcuts')
        );
        if (shortcutsBtn) {
            act(() => {
            ...
            await waitFor(() => expect(document.body.textContent).toContain('Keyboard Shortcuts'), {
                timeout: 3000,
            });
    ...
    it('AppShell (mobile): isMobile=true renders MobileAppBar + BottomNav + MoreSheet (L134/L160 true branches + L162/L163)', async () => {
        ...
        await waitFor(() => expect(screen.queryByText('More')).toBeTruthy(), { timeout: 3000 });
    ...
    it('RouteGuard: isLoading=true renders BrandedFallback (L181 true branch)', async () => {
        ...
        await waitFor(() => expect(document.querySelector('img[alt="Atlas"]')).toBeTruthy(), {
            timeout: 3000,
        });
        resolveSettings(undefined);
    });
