...
interface Props {
    ...
    ownerName: string;
    ownerAccent: string;
    /** Optional callback wired to the empty-state "New Task" button. When
    ...
    onPageSizeChange?: (size: TaskTablePageSize) => void;
    page?: number;
...
export function TaskTable({
    ...
    const [internalPageSize, setInternalPageSize] = useState<TaskTablePageSize>(20);
    const [internalPage, setInternalPage] = useState(1);
    const pageSize: TaskTablePageSize = pageSizeProp ?? internalPageSize;
    const page = pageProp ?? internalPage;
    const setPageSize = useCallback(
        (next: TaskTablePageSize) => {
    ...
    // Stable per-row click handler so TaskRow memo bails across parent renders.
    const handleOpen = useCallback((id: string) => navigate(`/tasks/${id}`), [navigate]);
...
interface PaginationFooterProps {
    ...
    onPageSizeChange: (next: TaskTablePageSize) => void;
}
    ...
    return (
        ...
        >
            <Typography sx={{ fontSize: 12, color: ATLAS_PALETTE.slate60 }}>
                {totalRows} task{totalRows === 1 ? '' : 's'} · page {page} of {pageCount}
            </Typography>
            <Box sx={{ flex: 1 }} />
            <Box
                component="label"
