...
describe('LabelsFormField', () => {
    it('renders existing labels as chips', () => {
        renderWithProviders(
            <LabelsFormField labels={['frontend', 'urgent']} onChange={() => {}} suggestions={[]} />
        );
    ...
    it('shows placeholder when there are no labels', () => {
        ...
        expect(screen.getByPlaceholderText(/type a label and press enter/i)).toBeInTheDocument();
    });
    ...
    it('fires onChange with a deduped, trimmed, capped list when a new chip is added via Enter', () => {
        ...
        renderWithProviders(
            <LabelsFormField labels={['frontend']} onChange={onChange} suggestions={[]} />
        );
