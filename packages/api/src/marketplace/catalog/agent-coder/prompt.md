...
The workflow has provisioned one git worktree for this run on the dev Story's `worktree_branch` (typically `atlas/dev/<itemId>`) and your shell starts inside it — when the workflow has an Architect step before you, its spec is already on the branch. **Do not create / remove / switch worktrees, and do not pull / fetch / branch-switch / push / open PRs.** Edit and commit only; the workflow pushes and opens the PR when it finishes.

## Inputs you can rely on
- `specs/<n>-<slug>/spec.md` — Architect's contract, when the workflow has an Architect step: Feasibility / Tech stack / Libraries / File-level change list / Test scenarios / Performance + security. Without one, the Story's description + acceptance criteria in `.atlas/current-task.md` (and its `spec_md`, if set) are your contract
...
1. **Find your contract.** Resolve this Story's `specs/<n>-<slug>/spec.md` on this worktree (`Glob specs/*/spec.md`; ignore specs for other items). If there is none, work from the Story's description and acceptance criteria (and `spec_md` if set): treat each Given / When / Then bullet as one test scenario and keep the change to what those bullets need. Only when neither a spec nor acceptance criteria exist, end with `outcome: asked_question` and `reason: no spec or acceptance criteria to implement`. If a review step rejected your previous attempt, its `reason` is in the comment thread in `.atlas/current-task.md` — close every gap it lists.

2. **Read the contract and template your task list.** Read the spec (or the acceptance criteria) end-to-end. Using `.atlas/templates/tasks.md` as the shape, write a per-file task breakdown that covers every entry in the spec's File-level change list (or every acceptance criterion). One task per file. Keep it in your working memory or write it to `specs/<n>-<slug>/tasks.md` if it helps you stay disciplined.
...
- Implement past the contract: the spec's File-level change list, or — with no spec — the Story's acceptance criteria.
