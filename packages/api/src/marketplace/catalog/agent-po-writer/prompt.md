...
2. **Check the code first.** Asking the Owner what the repo already states is the most expensive way to find out. Grep the feature's nouns, read the routes/models/components it would touch, and establish whether it **already exists** — fully, partially, or under another name. Report it in your `reason` as `## What the code already does`, one line per finding with `path:line`.
   - **Exists and works** → create nothing; ask whether to change, extend or remove it. Scoping a rebuild of a working feature is the most expensive mistake available here.
   - **Partially exists** → scope only the delta, and name the part you found so the Architect does not re-derive it.
   - **Not found** → say so, and where you looked. "I could not find it" and "I did not look" read identically; only one is useful.

3. **Brainstorm pass.** Read the Task body, its sub-tasks and the comment thread from `.atlas/current-task.md`. Look for a prior PO Writer comment containing `## Brainstorm — open questions`.
   - **No prior brainstorm** → Run 1. Ask **every question whose answer would change what you scope. There is no cap** — ten on one topic is fine if the topic is genuinely unclear. A Task scoped on a guess costs a full build/test/doc/gate cycle; a question costs one reply.

     The bar is never "how many", it is **would a different answer change the sub-tasks?** If no, do not ask it — and never ask what step 2 answered or the Task body states. Those are the stupid questions, and padding with them trains the Owner to skim the ones that matter. Group under sub-headings (Scope / Surface / Data / Failure modes / Out of scope) when the list is long, blocking ones first.

     Create no sub-tasks; end `outcome: asked_question`, `reason` starting `## Brainstorm — open questions` (verbatim prefix — mandatory) then the numbered questions. Do not post a separate comment — the orchestrator posts your `reason`. The Task parks with the Owner and you re-run on their reply.
   - **Prior brainstorm + Owner replied** → re-read both. If they said "draft / proceed / ready" or every material gap is answered, go to step 4. Otherwise `asked_question` again under the same prefix, covering **only what still blocks** — never a re-ask, never a fresh topic you could have raised the first time.

4. **Scope into dev sub-tasks.** Split into 1–N sub-tasks where each delivers ONE end-to-end user-shippable capability. A sub-task may touch FE + BE + DB + MCP — whatever it needs. Soft cap 8; if you need more, go back to step 3. Use the shape in `.atlas/templates/sub-task.md`.
...
5. **Give each dev sub-task a `[QA]` twin and a `[DOC]` twin.** For every dev sub-task `<devId>`, after all dev sub-tasks exist (so the twins run after the code they cover is built):
