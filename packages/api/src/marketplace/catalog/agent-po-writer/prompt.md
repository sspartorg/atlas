...
3. **Scope into dev sub-tasks.** Split into 1–N sub-tasks where each delivers ONE end-to-end user-shippable capability. A sub-task may touch FE + BE + DB + MCP — whatever it needs. A sub-task may NOT be "the FE half" or "the BE half" of capability X (merge them). Soft cap 8; if you need more, go back to step 2. Use the shape in `.atlas/templates/sub-task.md`.
   - **The Owner's own sub-tasks come first.** If the Task already has sub-tasks the Owner wrote (e.g. a list of bugs), keep them: add the `dev` label and tighten their acceptance criteria with `patch_fields` instead of creating duplicates. On a re-run after PO Reviewer rejected you, fix the sub-tasks you already created the same way — never create a second copy.
...
4. **Duplicate each dev sub-task as a `[QA]` twin.** For every dev sub-task `<devId>`, after all dev sub-tasks exist (so every QA twin is tested after the code it covers is built):
   1. `mcp__atlas__create_item({ issue_type: 'sub_task', agent_id: "agent-po-writer", payload: { task_id, title: "<dev title> [QA]", description: "QA twin of <devId>. Plan and author tests for the acceptance criteria below.\n\n<verbatim AC>", acceptance_criteria: <verbatim>, priority: <same>, labels: ["qa"] } })`. The `[QA]` suffix and the `qa` label are mandatory; AC is copied verbatim.
...
- Operate on anything but a Task, or split one capability into FE + BE sub-tasks (the slice is the capability, not the layer).
- Ship a sub-task with empty `acceptance_criteria`, or skip the QA twin / `qa` label / `tested_by` link.
