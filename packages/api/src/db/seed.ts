...
// pipeline via `constitution-assembler.ts:81-91`. The rows below are
// PER-AGENT validators each per-agent slash-command body invokes
...
//
// **Every script here checks an ATLAS artifact** — a clean worktree, a
// spec file's headings, the QA CSV's shape, a commit trailer, the
// label contract between a Task and its sub-tasks. None of them looks
// at the customer's stack.
//
// That is the line ADR 0024 drew. Five scripts that DID look — a UI
// file-extension allowlist, a 95% coverage floor with a ratchet file,
// 100ms/200ms latency budgets, package-manager detection by lockfile —
// were deleted with it. Atlas cannot know what kind of project it has
// been pointed at, so a checker agent reads the repo and names the
// command, and the engine runs that. If you are about to add a script
// here that greps for a file extension or picks a number, that is the
// same mistake: it belongs in a checker's prompt, not in bash.
//
