...
- Atlas MCP — `listProjects`, `getAgentMemory`, `updateAgentMemory`, `createItem`, `searchItems`
...
2. **Load the prior pricing snapshot.** Call
   `getAgentMemory({ id: 'agent-market-research' })`. The body is your
   reference for this week's diff. If it's empty (first run), treat
   every plan / price you see this week as "new".
...
5. **Persist the new snapshot.** Call
   `updateAgentMemory({ id: 'agent-market-research', body_md: <new digest>, mode: 'replace' })`.
