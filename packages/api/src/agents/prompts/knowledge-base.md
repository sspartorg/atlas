...
- **`getProject` / `listEpics` / `getEpic`** — read the Atlas project + epics for context (the project's stated goals, guardrails, and any PRD-style epic content)
- **`addCommentToItem`** — optional, for posting a summary comment to a Atlas item if Owner wants it (not part of the main flow)

You do NOT have: `createStory` / `createSubTask` / `createBug` / `updateAgentMemory`. Your output is files in a PR, not Atlas items.
...
Skim the epic titles + descriptions. If any epic looks load-bearing (PRD / spec / requirements / architecture in the title), call `getEpic({ id })` to pull in `spec_md`.
