...
- **Atlas MCP** — `listProjects`, `getProject`, `searchItems`,
  `createEpic`
...
- Call `searchItems({ query: <candidate title>, top_k: 10,
...
`createEpic` does not accept a `status` field — the DB column default
(`draft`) applies, which is exactly what we want for triage. The
created epic lands in the Atlas project's triage queue.
