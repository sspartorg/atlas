...
goTo(node after Start)
   ...
   └─ end node → End gate (Task runs with Sub-tasks nodes): open claimed sub-task → back to its
                  ...
                  finishRun: commit leftovers · ADR 0020/0024 verification gate per repo (runs
                  `project_repos.verify_command` in that checkout; fail, needs_review OR
                  unavailable — including an unset command — skips the repo and parks at End
                  with its output) · push run branch (push_code)
...
- **Prompt** — `prompt-builder` renders the constitution, the role with `{{ key }}` substitution against `settings_json` and the outcome contract, then either `# Project Context` (name, description, guardrails, every Task + spec, commit discipline) when the workflow has a project, or a `# Project-level Run` paragraph when it doesn't, then output instructions and self-memory. A **gate step** (ADR 0021, 0024) spawns its **checker agent** like any other step the first time it is entered: the checker reads the repo and emits `applies` + `command` in its `atlas-outcome` block, and Atlas then runs that command per repo, writes the verdict and the command to `run_gate_results`, and takes the pass edge on exit 0 or the fail edge (to a fixer agent, with the command's output posted on the item as its contract) otherwise. A later entry — a fixer looping back — re-runs the memoised command with no second dispatch. `applies: false` records `skipped` and passes; a checker that names no command parks the run, because absence of evidence is never a pass (ADR 0020). Atlas ships no thresholds, no probes and no file-extension heuristics: a project that declares no perf or visual tooling is recorded as `skipped`, not measured against a number Atlas picked. Agent steps read `.atlas/outcome.md`, `.atlas/self-memory.md` and `.atlas/changed-files.md` from the working directory — the last is the branch diff against the merge-base, staged so an agent does not rediscover it by crawling the repo.
