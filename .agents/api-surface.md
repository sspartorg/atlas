...
| GET | `/api/repos` | **ADR 0017.** Every repo of every project → `IProjectRepo[]` (`projectReposService.listAll`): per project its primary repo (`id` = the project id), then its `project_repos` extras. Each carries `project_id`. Feeds the Jira tab's Sources picker. |
