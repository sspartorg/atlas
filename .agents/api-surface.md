...
| POST | `/api/comments` | Add comment. **2026-09-14 — Owner reply resumes the agent:** an Owner-authored comment on an item that is `waiting_for_info` with `assignee_agent_id IS NULL` hands it back, in the comment's transaction, to the agent of the item's most recent `agent_runs` row (if that agent still exists and is `active`): sets `assignee_agent_id` + `status='ready'`, writes `assigned` and `status_changed` `issue_events` with `actor_agent_id=null` and `detail='resumed_by_owner_reply'`, then broadcasts `counts_changed {issueType, issueId}` (same as the assign/transition services). The agent-schedule-registry's dispatch-on-ready starts the run on its next tick. Agent-authored comments never trigger it. Implemented in `services/comments.ts::resumeParkedItem` |
...
| `counts_changed` | services/stories, issues, epics, **agents**, **projects** on create/update/transition/assign/delete; services/comments on an Owner reply that resumes a parked item; services/external-links when a PR's `pr_state` changes; notifications on create/markAllRead; agent-runner on run queue; routes/marketplace on install | any DB mutation that could affect sidenav badges or dashboard KPIs |
...
| File | Purpose |
|---|---|
| `agent-runner.ts` | Spawn `claude` / `copilot` CLI (`ollama` -> `claude` + env overlay); build prompt; stream SSE; auto-advance status. Advances item `ready â†’ in_progress` at spawn and `in_progress â†’ waiting_for_info` on error. On completion (Theme 04) consults `agent_handoff_rules` and shifts `assignee_agent_id` to the on-pass target (a crashed run parks with the Owner as `waiting_for_info` and ignores the on-fail rule since 2026-09-14, because reviewer on-fail rules now point at the writer); only the assignee shifts â€” status remains the status machine's domain. Has simulated mode when `ATLAS_AI_ENABLED=false` (line 197). **B04 â€” pre-dispatch depends_on gate:** `spawnAgentRun` calls `assertDepsAllDoneForDispatch(itemId, agentId)` before any `agent_runs` insert. Throws `DependenciesNotReadyError` (`blockers: [{id, title, status}]`) when any outgoing `depends_on` target is non-`done` (including `in_review`); writes a `dispatch_blocked` `issue_events` row. `POST /api/run` surfaces as `409 {error: 'dependencies_not_ready', blockers}`; `agent-dispatcher.maybeAutoDispatch` returns `{dispatched: false, reason: 'deps_blocked', blockers}`. Reviewer-leg + performer-retry helpers re-check defensively. **T2/T4 â€” worktree provisioning**: when the item carries a `worktree_branch` OR the agent's `role_id` matches a `ROLE_BRANCH_OVERRIDES` entry, `spawnAgentRun` calls `ensureWorktree({ item, project, agent })` from `worktree-orchestrator.ts` to spawn / pull a per-item worktree, narrows `cwd` to it, and prepends a `buildWorktreePreamble(...)` block to the agent prompt so the model knows not to run `git worktree add` / `git pull` itself. **2026-09-14 — run isolation:** item-driven (`requires_item = true`) claude + ollama runs add `claudeIsolationArgs(agent)` (empty for freedom-mode scouts, which need the Owner's Playwright / claude.ai connectors) = `--setting-sources project,local --strict-mcp-config --mcp-config {"mcpServers":{"atlas":{"type":"http","url":ATLAS_MCP_URL}}}` (`ATLAS_MCP_URL` exported from `plugins/mcp-host.ts`, `http://127.0.0.1:4500/mcp`, no auth header, since the MCP host checks none). This keeps the Owner's `~/.claude` hooks, plugins, user CLAUDE.md and `~/.claude.json` MCP servers out of runs, while worktree `.claude/commands/atlas-*` still load. It was verified live: no `hook_started` events, `plugins: []`, `mcp_servers: [{atlas, connected}]`. The `mcp__playwright` / `mcp__claude_ai_Atlassian` allowlist entries therefore resolve to nothing. `--mcp-config` is variadic, so the prompt must stay on stdin. Copilot is unchanged. **Child env:** `agentRunEnv(cli, model, gitConfigPath, ghToken)` = `gitInvokeEnv` + `ATLAS_API_URL=http://127.0.0.1:<apiPort()>` (`config.ts`, API_PORT → PORT → 4001) + the ollama overlay (last), so `.atlas/scripts` validators can call the API. **Rounds:** the self-routed and `apply_on_fail` branches call `resetRoundsUnlessBounceBack`, so rounds reset only on forward progress. |
| `worktree-orchestrator.ts` | T2 â€” non-AI git worktree orchestrator. `ensureWorktree({ item, project, agent? })` resolves the on-disk path (`<project.git_path>/../worktrees/<projectId>/<branchSlug>`), spawns / pulls the worktree, and persists the resolved path back to `items.worktree_path`. Auth: builds a per-call `GIT_CONFIG_GLOBAL` with `http.extraheader = AUTHORIZATION: basic <b64>` when `project.credential_id` is set so shell-outs bypass every credential helper. **T4 â€” role-aware branch override**: the exported `ROLE_BRANCH_OVERRIDES` map keys `role_id â†’ (itemId) => branchName`. Currently only `automation: itemId => 'atlas/auto/<itemId>'` so the Automation Engineer ALWAYS runs on a fresh branch cut off latest `main` (the dev PR has merged by then; reusing the QA writer's branch would stack commits on a stale base). Without an override entry, the orchestrator uses `item.worktree_branch` as-authored by PO Writer. Errors surface as `WorktreeProvisioningError` with `code: 'missing_worktree_branch' | 'missing_project_git_path' | 'invalid_branch_name' | 'git_command_failed'`. **2026-09-14 — PR gate:** `agent-runner` calls `openPullRequest` only when `raises_pr && exit 0` AND `shouldOpenPullRequest` (`agent-runner-outcome-routing.ts`) approves: never when the item was parked in `waiting_for_info` (reviewer rejection via MCP) or the outcome block is `rejected` / `asked_question`, nor when the agent neither signalled nor routed; project-scope runs are unaffected. Previously a rejecting reviewer still opened a PR. **2026-09-14 — scratch `.gitignore`:** `ensureWorktreeGitignore` now commits the appended Atlas-scratch patterns as their own commit, `chore(atlas): ignore Atlas scratch paths` (pathspec commit of `.gitignore` only, via `runGit` with the same `gitConfigPath` / `[user]` identity). They no longer ride inside the agent's first commit. Best-effort: if the commit fails (for example a project hook rejects it), the file stays staged as before. |
| `agent-handoff.ts` | `resolveHandoffAssignee(agentId, kind)` reads `agent_handoff_rules`, resolves the `'owner'` sentinel to `assigneeId: null`. **2026-09-14:** if the target agent is not installed or not `active`, the outcome becomes `{assigneeId: null, status: 'waiting_for_info', unavailableTarget}`, and `applyOnPassHandoff` / `applyOnFailHandoff` record `detail: 'handoff_target_unavailable: <slug>'` on the `status_changed` / `assigned` events. Before this, the handoff hit an `items.assignee_agent_id` FK error. `inferAssignee(explicit, reporterAgentId)` is the helper child-item create() functions use: caller's explicit value always wins; otherwise inherits the reporter's on-pass target. The handoff rule's `status` field is read but not applied by Theme 04 â€” Theme 06 may make it a filter hint or an actual transition target. |
| `agent-dispatcher.ts` | `maybeAutoDispatch(itemId)` â€” pure `shouldAutoDispatch()` precondition check + IO-bound spawn if `(item.status='ready', has assignee, agent active, no live run)`. Called **only** from `agent-schedule-registry.ts`'s periodic tick, which checks item-driven agents every minute (dispatch on ready, 2026-09-14) (owner wants scheduler-driven dispatch â€” transition/assign do NOT dispatch). Returns `{dispatched: true, runId}` or `{dispatched: false, reason}`. **2026-09-14 — max_rounds loop guard:** after the live-run check, if `getRound(item, agent) >= agents.max_rounds` it parks the item (assignee null, `waiting_for_info`, `status_changed` + `assigned` events with `detail: 'max_rounds_reached: <agent> (n/max)'`, `counts_changed` SSE) and returns `reason: 'max_rounds_reached'`. Manual `POST /api/run` is not capped. This is the only place the cap is enforced. |
| `handoff-assembler.ts` | `assembleHandoff({ worktreePath, agentId })` writes `.atlas/handoff.md`: required checklist + on-pass / on-fail routing steps (comment → change_status → assign, with a park-with-Owner fallback) read from `agent_checklists` / `agent_handoff_rules`. **2026-09-14** — every `mcp__atlas__update_item` step (primary, fallback, no-rule) instructs the agent to pass `agent_id: "<its id>"`, because the in-process MCP host has no bound agent id and un-credited writes were Owner-attributed (plus a duplicate "assigned" row from `otherActorReassignedDuringRun`). Known limitation: item create/link events from MCP remain Owner-attributed. **2026-09-14 — on-fail to an agent** (a reviewer handing back to its writer): step 1 becomes "FIRST post the gap list … BEFORE reassigning", because the writer is auto-dispatched on assign and works from that comment. An extra **Owner-only blocker** block (comment → assign null → `Waiting for Info`) covers failures only the Owner can fix. On-fail to `owner` renders neither. |
...
| `agent-schedule-registry.ts` | Single clock-driven poller (1-min cron), anchored at local midnight. `reseedAllActiveAgentsOnBoot()` re-anchors every active agent's `next_run_at` to its next slot from `now` on every server restart; item-driven agents fire whenever items are `ready` AND capacity > 0 (every tick); freedom-mode agents fire when `next_run_at <= now` AND capacity > 0. |
...
| `comments.ts` | Threaded comments per issue. An Owner comment on a parked (`waiting_for_info`, unassigned) item reassigns the last-run agent and re-queues it to `ready` (see `POST /api/comments`). |
