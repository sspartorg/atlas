...
| POST | `/api/workflows/from-template` | `{ template_id, project_id }`. Installs missing catalog agents (`marketplaceService.install`), resolves End `child_workflow_id: "template:<id>"` to that template's workflow **by name in the same project** (creating that workflow from its template first when the project lacks it), then creates. 201 |
