...
| POST | `/api/workflows/import` | Token-gated. Multipart (`project_id` field **before** the `file` part — only earlier fields are visible to `req.file()`; `?project_id=` also accepted) or raw `application/zip` + `?project_id=`. → `IWorkflowImportResult` `{ workflow, sub_workflows, installed_agents, reused_agents, agents }`, 201. `agents` is an `IAgentDependencyReport` `{ installed, upgraded, skipped_edited, unchanged }`: import no longer merely SKIPS an agent that already exists. A back-linked agent behind the catalog is brought up to date, unless the Owner has edited its prompt — those are left untouched and reported under `skipped_edited`, with the pointer deliberately left behind so the marketplace still offers the upgrade. Owner-edited is decided by `marketplaceService.hasOwnerPromptEdit`: an `agent_prompt_versions` row with `edited_by = 'Owner'` past v1 (install seeds v1 as `'Owner'`, `acceptUpgrade` writes `'Marketplace'`). 400 `Workflow bundle: …` for a malformed zip (not a zip, missing/invalid `workflow.json`, a `format_version` above this Atlas's (`made by a newer Atlas (format N); update Atlas to import it`), a missing `workflows/<ref>.json` or `agents/<id>/manifest.json`, an agent manifest that fails its schema); 400 no `project_id`; 404 unknown project; 400 graph / delivery / schedule errors from `workflowsService.create`; 400 when a bundled agent's `(cli, model)` isn't in `cli_models` |
