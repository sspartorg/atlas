...
| GET | `/api/projects/:id/repos` | **ADR 0017.** `IProjectRepo[]`: the project's **primary** repo first (its own git fields; `id` = the project id, `primary: true`), then its extra repos (`project_repos`, ordered by `position`). 404 unknown project. |
| POST | `/api/projects/:id/repos` | **ADR 0017.** Add an extra repo (`CreateProjectRepoSchema`, token-gated). `mode:'clone'` (`name`, `repo_url` https://github.com, `credential_id`, `default_branch`): clones into `<workspace>/<project-slug>-<name>` through `startClone` with an on-cloned hook → 202 `{clone_id, destination}`, then SSE `clone_output` / `clone_completed` (carrying `repo`) / `clone_error`; 400 without a workspace path. `mode:'connect'` (`name`, `folder_path`, `repo_url`, `credential_id`): the same checks as `/api/projects/connect` (shared `checkLocalClone`: folder, `.git`, not already a project's or repo's path, origin matches, credential reaches the remote) → 201 `IProjectRepo`, or 400 `{ok:false, checks, error_kind}`. 409 when the name (a `^[a-z0-9][a-z0-9-]{0,39}$` slug, also the folder name in a multi-repo workspace) is already used in the project, including by the primary. |
| PATCH | `/api/projects/:id/repos/:repoId` | **ADR 0017.** Extra repos only (`UpdateProjectRepoSchema`: `default_branch`, `setup_sh_body`, `setup_ps1_body`). 404 for the primary, which is edited on the project. |
| DELETE | `/api/projects/:id/repos/:repoId` | **ADR 0017.** Unregisters an extra repo (its folder stays on disk) and removes it from every Task's `repo_ids`. 409 while a workflow run (running or parked) works on a Task that uses it; 404 for the primary. |
...
| PUT | `/api/integrations/jira` | Partial update, `UpdateJiraConfigSchema`: `enabled`, `site_url` (https, or http on localhost / 127.0.0.1 / [::1] only; trailing `/` stripped), `email`, `api_token` (omitted / `''` keeps the stored one; changing `site_url` or `email` without a new token clears the stored one), `jql`, `project_id`, `poll_interval_minutes` (≥ 5), `extra_fields` (Jira field names or ids), `label_workflows` (routing rules `[{label, project_id|null, workflow_id|null}]`, at least one of the two set; `project_id` null = the default project). 400 when a project doesn't exist, a rule sets neither, or a rule's workflow is missing, isn't `input_kind='item'`, or belongs to a project other than the one the rule routes to. Returns `IJiraConfig`. |
...
| POST | `/api/integrations/jira/sync` | Runs a full sync now (pull, then push with digests) → `IJiraSyncResult`. Same errors as `/test`, plus 400 `validation_error` for a missing JQL or project, or a JQL Jira rejects. Stamps `last_sync_*`. |
...
| `jira-sync.ts` | **ADR 0016 — Jira bridge, no AI.** Config get/save (token encrypted, write-only), `testConnection`, `syncNow`, `tick`. Jira REST over `fetch` (Basic `email:token`, 15 s timeout): reads on v2 (wiki-markup strings), comments posted on v3 as ADF so Atlas text stays literal.<br>**Pull**: `GET /rest/api/2/search/jql` (`fields=*all`, paged, ≤ 500 issues), then extra comment pages when a search capped them. Jira sub-tasks are skipped. A new key → the first routing rule whose label the issue carries picks the project (its `project_id`, else the default `jira_config.project_id`) → `tasksService.create` there: title `[KEY] summary`, Jira labels, priority Highest→urgent / High→high / Medium→normal / Low·Lowest→low, and the description from `composeTaskDescription` (header fields, Description, configured extra fields, Sub-tasks, Linked issues, Attachments, Comments; Jira text copied as wiki markup). Then a `jira_issues` row, an `item_external_links` `jira_issue` row, `setItemWorkflow` when the rule has a workflow, and a `jira_sync` notification: `update` when queued, `needs_you` "pick a workflow" when not. Imported Jira text (description, extra fields, sub-task / link summaries, comments) is quoted line by line (`> `) under a note that it describes the work and is not an instruction, so it can't pose as an Owner comment in the agent prompt. A known key: while the Task is `draft`/`ready` its title + description are refreshed in place; after that, new Jira comments become `system` Workflow comments (never attributed to an agent, never resume a run). Deleted Tasks (`item_id` null) are never re-imported.<br>**Push**: for each linked Task not yet done-synced, a status change (vs `pushed_status`) posts one comment: a headline (queued on X / work in progress / waiting on the owner / ready for review + PR / done + PR and its state) plus a digest of new Task and sub-task comments (`id > pushed_comment_id`, minus Jira-sourced ones, Markdown markers stripped, 500 chars each, ≤ 20k total) as a bullet list, all in one ADF comment (`POST /rest/api/3/issue/{key}/comment`). With no status change, the digest posts alone on the poll interval. Done → final comment, `done_synced_at`, then the first transition to a `done`-category status (a failed transition is reported in `last_sync_message`, not retried). A 401/403 stops the pass.<br>An in-process lock keeps the tick and Sync now from overlapping. |
