...
| `engineer` | Engineer | yes | active | Coder — one dev sub-task at a time. Paired Code Reviewer agent (`build` sub-workflow; the Task's `delivery` run opens the one PR at End). Also **Fix Reviewer** — checks *how* each gate fixer made its check green, on all four gate fail paths in `delivery` v5. The Release Reviewer also files the fix sub-tasks its own rejections need. |
...
| `tester` | Exploratory Tester | yes | active | Coverage Fixer — writes the tests that close the gap the project's own suite reported (`delivery` v5); the threshold is the project's, never one Atlas picked (ADR 0024). Also **Tests Check**, the checker that names that command. No exploratory-testing agent ships. |
...
| `devops` | DevOps Engineer | yes | active | Performance Fixer — brings the touched routes back under the project's own budget after the performance check (`delivery` v5). Also **Performance Check**, the checker that finds that budget or reports there is none. |
| `security` | Security Review Lead | yes | active | Hygiene Fixer — closes what the project's own lint and typecheck command reported (`delivery` v5). Also **Hygiene Check**, the checker that names that command. |
| `designer` | UX/Visual Designer | yes | active | Visual Reviewer — reads the screens the project's own visual check captured, fixes overflow/contrast/theme, blesses a new baseline (`delivery` v5). Also **Visual Check**, the checker that finds that tooling or reports there is none. |
