...
**`trace_summary` (migration 017)** — what the run actually *did*, as opposed to what it reported about itself. `output_text` has held the full stream-json of every dispatch since the runner shipped and nothing server-side ever read it, so the only thing anyone could assert about a run was its own `atlas-outcome` block. `run-trace-parser.ts` reads that transcript **once, in `completeRun`**, into this jsonb column: `{ source, turns, tool_calls, tools{}, tool_sequence[], thinking_blocks, subagent_turns, files_touched[], errors, ttft_ms, truncated }`.

- Parsed at completion rather than on read: `output_text` runs to megabytes and the Tests tab polls every five seconds. Not its own table either — nothing queries an individual tool call, and every consumer wants the whole summary for one run.
- **Every field a CLI cannot report is `null`, never `0`.** Copilot emits `tool.execution_start` so names, counts, order and turns all survive, but it has no thinking event and does not surface tool arguments — so `thinking_blocks` and `files_touched` are null on a Copilot run. "We did not look" and "it did not happen" are different answers, and an expectation asserting on a field this CLI cannot answer must come back `errored` rather than silently passing (ADR 0020's rule for a gate that could not run).
- `files_touched` is relative to the run's own `cwd`, taken from the transcript's `system/init` event. Raw, the paths are absolute against a temp sandbox that changes every run — unmatchable by any expectation, and a leak of the machine's temp layout into product data.
- `ttft_ms` is measured from `agent_runs.started_at`, because `system/init` carries no timestamp of its own.
- **`NULL` on every run that finished before the migration** — there is deliberately no backfill, since reparsing every historical transcript is minutes of blocking multi-megabyte I/O to answer questions nobody is asking. Consumers render that as "—", never as zero.
- On the wire as `IRunTraceSummary` from `GET /api/run/:id` and `GET /api/agents/:id/runs`. **Deliberately `NULL::jsonb` in list mode** (`GET /api/run`): a trace carries up to 200 tool names and 100 paths, which over 500 rows is megabytes no list renders. Selected as an explicit NULL rather than omitted, because a column that `asAgentRun` maps but the query never SELECTs is a bug this route has already shipped twice.
...
Fields: `id, project_id, name, description, status, graph, input_kind, trigger, use_worktree, push_code, raises_pr, push_to_default, max_loops, max_parallel_runs, schedule_preset, schedule_time_of_day, schedule_weekday, cron_expr, next_run_at, last_run_at, marketplace_source_id, marketplace_pulled_version, marketplace_pulled_at, upgrade_available (computed), created_at, updated_at`
...
- **Templates** (`IWorkflowTemplate`, `packages/api/src/marketplace/workflows/*.json`): `delivery` (Task), `build` and `test` (sub-workflows), `ai-readiness` (project run). Each carries a `version` (migration 007; all at 1). Agent nodes reference catalog agent ids; a Sub-tasks node may name its sub-workflow as `template:<id>`, resolved (and created when missing) at create time. See `swarm-architecture.md`.
- `marketplace_source_id` / `marketplace_pulled_version` (migration 007) mirror the pair on `IAgent`: the template id (`delivery`) or published-entry id this workflow came from, and the version pulled. NULL means hand-built — not stale, just no upstream. Set only by the service, never by `POST /api/workflows`, because a client that could forge the source id could forge what upgrades apply. **Template sub-workflows resolve through this column, not by name**: name-matching forked on rename and adopted any same-named workflow as a build step whatever its graph held.
- `agents.marketplace_upgradable_hash` (migration 009) is the agent-side answer to the same question, in a different shape. An agent upgrade overwrites `prompt_md`, `settings_json` AND checklists, but only the prompt leaves an `agent_prompt_versions` row and checklists live in their own table — so an Owner who tuned a checklist read as untouched and an import deleted it. The hash covers those three fields and nothing else: `agents.updated_at` would also move on a rename or a pause, which an upgrade never touches. Null (installed before 009) reads as edited, so the automatic path declines instead of guessing.
- `marketplace_pulled_at` (migration 008) is when the upstream was last taken. **Untouched since pull** is `updated_at <= marketplace_pulled_at`, and that is what decides whether an import may upgrade a workflow automatically. `updated_at` alone cannot decide it — the `workflows_set_updated_at` trigger moves it on the upgrade's own write, which would freeze every upgraded workflow as "edited" forever. Both timestamps come from the DB clock in one statement; a JS timestamp, or a row mapper using `String(date)` (which drops milliseconds), makes an untouched workflow read as edited.
- `upgrade_available` is computed per read from the source's current version (`workflowsService.sourceVersion` resolves a template id, a published-entry id, or an `<entryId>:<ref>` sub-workflow). `POST /api/workflows/:id/upgrade` re-applies the source's description and graph — **explicit**, so it applies over local edits, exactly as accepting an agent upgrade does; name, schedule and delivery flags stay the Owner's. The automatic path (import) is the one that checks for edits first.
- `createFromTemplate` installs the template's **full agent closure**, its sub-templates' included (`workflowsService.templateAgentIds`). Scanning only the parent graph installed 4 of Delivery's 10 and the run parked on a missing agent.
...
- A workflow eval's samples run **one at a time**: `startWorkflowRun` provisions a worktree under the project git lock, so taking them in parallel would have them fighting over it. An agent test's samples still run together — each gets its own `mkdtemp` directory.
- A workflow eval is judged on `workflow_runs`, not on any one dispatch: terminal status, sub-task count, PRs opened, and gate verdicts across the whole tree (ADR 0015 child runs included). Its `cost_usd` is the delivery's total and its `duration_s` the run's wall clock.
- **A parked run is not finished.** Every fixture parks once at PO Writer's brainstorm by design, so the verdict waits for the Owner's answer.
- `suite` is a **tag, not an entity**. Running a set is running its fixtures; comparing two fleet versions is reading the same suite filtered by the `label` each run carried. A suite table earns itself when suite runs need their own lifecycle.

- `item_template` = `{ issue_type, title, description?, acceptance_criteria?, labels? }`.
- `expectations` has two halves. **What it said**: `outcome_kind`, `required_checklist_all_passed`, `summary_contains`, `summary_omits`, `max_cost_usd`, `max_duration_s` — judged by the pure `evaluateAgentTest` (`services/agent-tests-evaluate.ts`), which replays `decideRunRouting`, the same function the workflow engine routes on, so a test and the engine can never disagree. **What it did** (migration 017): `tools_required`, `tools_forbidden`, `max_turns`, `max_tool_calls`, `files_touched`, `files_untouched` — read from the run's own transcript rather than from the block the agent wrote about itself. `tools_forbidden: ["Edit","Write"]` on a reviewer is the canonical one: a reviewer that edited the code under review was not reviewing it.
- **An expectation that cannot be answered is `errored`, never a pass.** A trace expectation on a run from before migration 017, or a `files_touched` assertion against Copilot (which reports tool names but not their arguments), returns `errored` with the reason. Silently passing an assertion nobody could make is exactly the hole ADR 0020 closed when it separated a gate that went green from a gate that could not run. A truncated file list can still prove a file WAS touched, but never that one was not — the entries it dropped are precisely the ones in question — so `files_untouched` against a truncated trace also errors.
...
- Judged at completion (`agent-tests-evaluate-run.ts`, called by `completeRun` and `errorRun`), with a fallback on read for a run the API missed by crashing. Before that hook existed a run nobody opened stayed `running` for good, and its verdict depended on whether anyone was watching.

**Sampling (migration 018).** An agent is stochastic, so one dispatch decided a verdict a second press could have reversed: a test that passes three times in five printed whichever of "passed" and "failed" the Owner happened to press. One press of Run is now a **batch** of `n` samples (`batch_id`, `sample_index`, cap 10), each materialising its own throwaway item — a second dispatch against an item the first already changed is measuring something else. Samples run in parallel, which is safe specifically because an agent test calls `spawnAgentRun` with no workflow run, so each gets its own `mkdtemp` directory and never takes the project git lock.

- **No `agent_test_batches` table.** A batch has no state a row does not already carry and is derivable by `GROUP BY`; a table would need its own lifecycle, terminal-status logic and cleanup for data with none. Every run predating the migration was backfilled to a batch of one, so nothing downstream needs a "before sampling" case.
- The aggregate (`agent-test-batches.ts`) is **computed, never stored** — a stored verdict goes stale the moment one sample is re-judged. It reports `pass_at_1` (comparable with all pre-sampling history), `pass_at_k` ("can it do this at all"), `consistency`, `flaky`, and a `failure_histogram` saying *which* expectation was unstable and in how many samples.
- **`errored` never enters the denominator.** A CLI that was not installed is not an agent that got it wrong — the distinction `agent-tests-evaluate.ts` draws for one dispatch, kept at batch level instead of averaged away.
