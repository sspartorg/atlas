...
| Repos | `repos` | `ProjectReposCard` (`pages/project/`) — the project's git repos (ADR 0017), primary first. Each row: name (mono), **Primary** chip on the primary, default-branch chip (mono), clone status, `git_url` link. **Add repo** opens `AddRepoDialog`. Extra repos get **Edit** (pencil → `EditRepoDialog`) and **Remove** (bin → confirm); both are hidden on the primary, whose setup scripts live on the Setup tab (the card caption says so) |
