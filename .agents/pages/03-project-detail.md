...
| Jira | `jira` | `ProjectJiraCard` (`pages/project/`) — this project's Jira **sources** (migration 010), moved off the global Settings list. Each row: the JQL (mono), the workflow name ("No workflow — you pick one" when unset) and a chip per repo, with an **Actions for source N** menu (**Edit**, **Remove**). **Add source** / **Edit** open `JiraSourceDialog`: JQL (multiline, mono), a **Workflow** select restricted to `input_kind='item'` workflows that are global or this project's, and a multi-select **Repos** (pre-filled with every repo — a Task must name at least one). **Sync now** reuses `POST /api/integrations/jira/sync` and is disabled until a token is stored **and the Import switch is on** (G-014); an info alert links to Settings → Jira when no token is stored, and a separate warning alert ("Import is switched off") shows when the site is connected but the bridge is off — sources look configured while nothing is polled and no progress reaches Jira. Ordering matters: an issue matching sources in two projects becomes ONE Task under the **lowest-id** (first-created) source, and editing never re-orders. Empty state: "No Jira sources yet" |
