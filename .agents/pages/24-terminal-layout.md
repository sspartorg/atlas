...
**Terminal pane body** — the existing `TerminalXterm` component renders inside each attached pane. Its FitAddon + ResizeObserver already react to container resizes, so dragging a divider re-fits xterm without any backend change.
