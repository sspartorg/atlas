...
**Terminal pane body** — the existing `TerminalXterm` component renders inside each attached pane. It is a raw byte pipe: server PTY bytes go straight into xterm.js's stateful parser (no client-side decode/buffer), and the first frame after each (re)connect is a serialized screen snapshot from the server-side headless mirror, so refresh/reconnect repaints cleanly. Its FitAddon + ResizeObserver react to container resizes — `fit()` runs immediately so the pane tracks a divider drag, while the `{cmd:'resize'}` send to the server is trailing-debounced (~100 ms) so a drag doesn't storm ConPTY with reflows.
