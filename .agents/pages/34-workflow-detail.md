...
- **Canvas** (`WorkflowCanvas.tsx` + `WorkflowNodes.tsx`): dotted background, MiniMap, zoom Controls, fit view on load. The flow runs **top to bottom**; node positions anchor the node's top centre (`nodeOrigin [0.5, 0]`), so cards and pills in one column line up. Edges are right-angle `smoothstep` lines (also while drawing).
  - Every node but Start takes input on its **top** (`in`) and has a faint side entry on its **right** (`loop`, full opacity on hover). `routeEdges` (`graph.ts`) routes an edge into the target's `loop` unless the target sits a **clear node-height (64px) below** the source, else into `in` — loops run down the side instead of crossing the main line. The clearance matters: an equal-row target (a fixer and its reviewer) or a near miss (a fixer 60px under its gate) used to take the top handle, which made the edge exit right, double back over its own card and drop in from above. Handle choice is by live node position only; it isn't saved, and it is recomputed on every render so dragging a node re-routes its edges immediately.
  ...
  - **Lanes.** Every edge using a node's right flank — a `fail` leaving it, any edge entering `loop` — shares one corridor beside the column, and `smoothstep`'s default 20px offset drew them all on the same line. `routeEdges` colours those spans greedily by interval and gives each its own `pathOptions.offset` (20/38/56/74/92, capped inside the 104px gutter between columns). Derived per render from node positions, never persisted.
  - **Focus.** Hovering or selecting an edge fades every other edge to `opacity: 0.12` (pure CSS `:has()` in the canvas `sx`; `elevateEdgesOnSelect` lifts a selected edge above its neighbours). This is how you tell which fail goes where when several run side by side.
  - **Tidy up** (`Panel`, top right, builder only, hidden on a phone) → `tidyGraph` (`layout.ts`): lays the graph out top to bottom, repair branches to the right, on a 320 x 140 grid. Never runs on load or save — one deliberate press, and the button becomes **Undo tidy** for one press back. See `architecture.md` for the algorithm.
