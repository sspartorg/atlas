...
| **Child workflow** | The End node's `child_workflow_id`. Items created during the run (children of the run's item, or stamped `created_by_workflow_run_id`) are queued for it as `ready` at End, e.g. Planning → Development. Children that test another item (an outgoing `tested_by` link, like `[QA]` twins) go to the End node's `test_child_workflow_id` instead when set, e.g. Planning → Quality. |
