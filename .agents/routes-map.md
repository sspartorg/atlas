...
| `/agents/:id` | `AgentDetail` (6 tabs: Overview, Prompt, **Tests**, Test Run, Runs, Memory — Handoffs was removed by ADR 0014) | `useAgent`, `useUpdateAgent`, `useAgentMemory`, `useSetAgentMemory`, `useRegenerateAgentMemory`, `useAgentTests`, `useAgentTestRuns`, `useAgentCostEstimate`, `useQuery(['runs', agentId])`, plus per-tab hooks | `GET /agents/:id`, `PATCH /agents/:id`, `GET/PUT /agents/:id/memory`, `POST /agents/:id/memory/regenerate`, `GET/POST /agents/:id/tests`, `GET /agents/:id/cost-estimate`, `GET /agent-tests/:testId/runs`, `POST /agent-tests/:testId/run`, `DELETE /agent-tests/:testId`, `GET /agents/:id/runs`, `GET /cli/availability`, `POST /run` (Run now dialog) | `DuplicateAgentModal`, `RunNowDialog`, `GlyphPickerModal` | "Reassign queue", "Save as run" (TestRun tab), "Diff view for v{n}" (Prompt tab), formatting toolbar in Prompt tab |
