...
| `/terminal/:id` | `TerminalSession` | `useCliSession(id)`, `usePauseCliSession`, `useResumeCliSession`, `useNavigate` (Pause/Resume/Stop now via `TerminalSessionControls`) | `GET /cli/sessions/:id`, `POST /cli/sessions/:id/{pause,resume,stop}`, WS `/api/cli/sessions/:id/stream` | `StopSessionModal` (via `TerminalSessionControls`) | â€” |
