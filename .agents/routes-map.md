...
| `/agents/marketplace` | `Marketplace` | `useQuery(['marketplace','list',q,category])`, `useNavigate`, `useToast` | `GET /api/marketplace/agents?q&category&limit` (via `api.marketplace.list`), `POST /api/marketplace/agents/:id/install` (bulk, via `runBulkInstall`) | `BulkInstallBar` (sticky) | No page doc yet (`.agents/pages/*.md`). Bulk install reports the failing ids + server reason and STAYS on the page with them selected; only a clean sweep navigates to `/agents`. |
